"""Per-size bar aggregation table for reports.

A "row" describes one dealer's stats for one size_g slice over the period:
median price, median premium %, min/max premium %, and % time held cheapest.

`_premium_for_bar` and `_premium_for_coin` are exposed for reuse by the
notable-events generator. Coins are reported as bullet highlights elsewhere
(see `notable.detect_best_coin_deals`), not as per-variant tables.
"""
